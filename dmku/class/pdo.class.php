...
class sql
{
    ...
    {
        try {
        	  $sqlExe="SELECT * FROM danmaku_list ORDER BY time DESC";
        	  if(!empty($key)){
        	  	$sqlExe="SELECT * FROM danmaku_list WHERE text like '%$key%' or id like '%$key%' ORDER BY time DESC";
        	  }
            $stmt = self::$sql->prepare($sqlExe);
            $stmt->execute();
